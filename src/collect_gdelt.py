...
def _load_batch_manifest(batch_dir: Path) -> dict[str, Any]:
    ...
    if manifest.get("version") not in {1, 2} or not manifest.get("batches"):
...
def write_batch_plan(
    ...
    *,
    ...
    for index, batch in enumerate(batches, start=1):
        ...
        article_output_name = f"{batch_id}.articles.jsonl.gz"
        ...
        manifest_batches.append(
            {
                ...
                "article_output_file": article_output_name,
    ...
    try:
        ...
        if args.plan_batches:
            ...
            def estimate_windows(candidate: list[dict[str, Any]]) -> int:
                sql = _build_query_for_args(
                    candidate, args, languages, result_level="articles"
                )
        ...
        if args.execute_batch:
            ...
            if manifest.get("query_result_level") != "articles":
                raise ValueError(
                    "This is a legacy summary-only batch plan; regenerate it with "
                    "--plan-batches --overwrite before collecting article metadata"
                )
            batch = next(
                (
                    item
            ...
            article_output_path = args.batch_dir / batch["article_output_file"]
            metadata_path = args.batch_dir / batch["metadata_file"]
            if (
                article_output_path.exists()
                and output_path.exists()
                and metadata_path.exists()
                and not args.overwrite
            ):
                prior_metadata = json.loads(metadata_path.read_text(encoding="utf-8"))
                if prior_metadata.get("collection_mode") == "article_metadata":
                    print(f"Batch already complete; skipping: {article_output_path}")
                    return 0

            sql_path = args.batch_dir / batch["sql_file"]
            sql = sql_path.read_text(encoding="utf-8")
            maximum_bytes_billed = int(manifest["batch_limit_bytes"])
            selected_event_ids = set(batch["event_ids"])
            batch_windows = [
                window
                for window in windows
                if window["event_id"] in selected_event_ids
            ]
            if len(batch_windows) != len(selected_event_ids):
                raise ValueError(
                    f"Batch {batch['batch_id']} does not match the current event registry"
                )
            rows, job_meta = execute_article_query(
                sql,
                args.billing_project,
                article_output=article_output_path,
            ...
            metadata = {
                ...
                "article_output": batch["article_output_file"],
            ...
            print(
                ...
                f"{job_meta['article_rows']:,} article rows -> {article_output_path}"
            )
            print(f"Wrote MSS summary: {output_path}")
